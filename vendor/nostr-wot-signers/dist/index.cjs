...
var nip46 = require('nostr-tools/nip46');
...
var PrivateKeySigner = class {
  constructor(sk) {
    ...
    __privateSet(this, _sk, typeof sk === "string" ? utils.hexToBytes(sk) : sk);
    if (__privateGet(this, _sk).length !== 32) {
      throw new Error("Private key must be 32 bytes");
    }
    __privateSet(this, _pk, nostrTools.getPublicKey(__privateGet(this, _sk)));
  }
  async getPublicKey() {
    return __privateGet(this, _pk);
  }
  async signEvent(template) {
    return nostrTools.finalizeEvent(template, __privateGet(this, _sk));
  }
  async nip04Encrypt(recipientPubkey, plaintext) {
    return nostrTools.nip04.encrypt(__privateGet(this, _sk), recipientPubkey, plaintext);
  }
  async nip04Decrypt(senderPubkey, ciphertext) {
    return nostrTools.nip04.decrypt(__privateGet(this, _sk), senderPubkey, ciphertext);
  }
  async nip44Encrypt(recipientPubkey, plaintext) {
    const conv = nostrTools.nip44.v2.utils.getConversationKey(__privateGet(this, _sk), recipientPubkey);
    return nostrTools.nip44.v2.encrypt(plaintext, conv);
  }
  async nip44Decrypt(senderPubkey, ciphertext) {
    const conv = nostrTools.nip44.v2.utils.getConversationKey(__privateGet(this, _sk), senderPubkey);
    return nostrTools.nip44.v2.decrypt(ciphertext, conv);
  }
};
_sk = new WeakMap();
_pk = new WeakMap();
...
_ext = new WeakMap();
function toBytes(sk) {
  return typeof sk === "string" ? utils.hexToBytes(sk) : sk;
}
...
var _Nip46Signer = class _Nip46Signer {
  ...
  static async fromBunkerUri(uri, opts) {
    const bp = await nip46.parseBunkerInput(uri);
    if (!bp) throw new Error(`Invalid bunker URI: ${uri}`);
    if (bp.relays.length === 0) {
      throw new Error("Bunker URI is missing at least one ?relay=wss://\u2026 parameter");
    }
    const clientSk = (opts == null ? void 0 : opts.clientSecretKey) ? toBytes(opts.clientSecretKey) : nostrTools.generateSecretKey();
    const inner = nip46.BunkerSigner.fromBunker(clientSk, bp, {
  ...
  static startNostrConnect(opts) {
    ...
    const clientSk = opts.clientSecretKey ? toBytes(opts.clientSecretKey) : nostrTools.generateSecretKey();
    const clientPubkey = nostrTools.getPublicKey(clientSk);
    ...
    const ready = (async () => {
      try {
        inner = await nip46.BunkerSigner.fromURI(clientSk, uri, bunkerParams, abort.signal);
        if (cancelled) {
  ...
  exportClientNsec() {
    return nostrTools.nip19.nsecEncode(__privateGet(this, _clientSk));
  }
...
exports.Nip07Signer = Nip07Signer;
exports.Nip46Signer = Nip46Signer;
exports.Nip55Signer = Nip55Signer;
exports.PrivateKeySigner = PrivateKeySigner;
exports.isNip07Available = isNip07Available;
exports.ndkSignerAsNostrSigner = ndkSignerAsNostrSigner;
exports.nostrSignerAsNdkSigner = nostrSignerAsNdkSigner;
