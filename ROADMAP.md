...
- [ ] **WoT-weighted report display** — once reports exist, let the client act on them client-side without any relay cooperation: subscribe to kind 1984 authored by pubkeys inside the user's WoT (reuse `src/lib/wot/engine.ts`), and blur/collapse content from an account several trusted people have reported. NIP-56's own suggested use. Reports from outside the WoT are ignored — anyone can mint keys and report in bulk.
