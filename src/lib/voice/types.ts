...
export interface VoiceSignalPayload {
  ...
  /** Opaque signaling generated by simple-peer (SDP, ICE, and renegotiation). */
  peerSignal?: unknown;
   ...
   *  through repeated redial attempts. */
...
export interface VoicePresence {
   ...
   * screen-share). Drives the independent room-wide caps: four cameras and
   * one screen share.
   * Every participant derives the same winners from live relay beacons.
   *
   * Race-overflow is resolved deterministically by `(createdAt asc, pubkey
   * asc)` sort across the flattened (publisher, kind, createdAt) list. Tracks beyond each
   * kind’s leading slice are auto-evicted by their owners.
   */
